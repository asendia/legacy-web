<script lang="ts">
	...
	$: {
		const key = disabled ? 'disabled' : ((variant + color) as keyof typeof colors);
		const backgroundColor = colors[key].background;
		const borderColor = colors[key].border;
		const textColor = colors[key].color;
		_style =
